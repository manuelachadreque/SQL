...
WHERE EXISTS (
		SELECT 1
			...
			WHERE B.PurchaseOrderDetailID =A.PurchaseOrderID AND OrderQty >=500
		)
ORDER BY 1
...
WHERE EXISTS (
		SELECT 1
			FROM  Purchasing.PurchaseOrderDetail B
			WHERE B.PurchaseOrderDetailID =A.PurchaseOrderID AND OrderQty >500 AND UnitPrice>50
		)
ORDER BY 1
...
WHERE NOT EXISTS (
		SELECT 1
			FROM  Purchasing.PurchaseOrderDetail B
			WHERE B.PurchaseOrderDetailID =A.PurchaseOrderID AND B.RejectedQty>0
		)
ORDER BY 1
